...
class BookRoom
{
    ...
    public static async Task<IResult> Get(int id, Config config)
    {
        string query = @"SELECT user_id, travel_id, room_id, check_in, check_out, total_price 
                         FROM booking_rooms WHERE id = @id";
    ...
    {
        string query = @"INSERT INTO booking_rooms (user_id, travel_id, room_id, check_in, check_out, total_price)
    ...
    // PUT
    public record BookingRoomUpdate(int Id, int UserId, int TravelId, int RoomId, DateTime CheckIn, DateTime CheckOut, decimal TotalPrice);
    ...
    public static async Task<IResult> Delete(int id, Config config, HttpContext ctx)
    {
        ...
        string query = "DELETE FROM booking_rooms WHERE id = @id";
        var parameters = new MySqlParameter[]
        {
            new("@id", id)
        };
